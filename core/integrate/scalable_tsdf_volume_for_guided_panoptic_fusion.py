...
class GuidedPanopticFusionScalabelTSDFVolume(PanopticFusionScalableTSDFVolume):
    ...
    _guidance_hash: Tensor = None

    def load_guidance_label(self, hash_pth: str, label_pth: str):
        label: Tensor = torch.load(label_pth, map_location=self.device).to(self.device)
        unique_label, label = label.unique(sorted=True, return_inverse=True)
        self._guidance_label = label  # id = 0 means uknown
        self._instance_label_num = unique_label.shape[0]

        hash: Tensor = torch.load(hash_pth, map_location=self.device).to(self.device)
        self._guidance_hash = hash
    ...
    def integrate_instance_with_existing_voxel_and_guidance(
        ...
        if depth_type == "sensor":
            ...
            layer_px, layer_py, layer_vol_idx = pixel_voxel_corres_given_depth(
                voxel_hash=self._guidance_hash,
    ...
    def fuse_label_into_voxel(
        ...
        # ! STEP 2, map 2d to 3d
        mapped_img = torch.zeros(size=(H_m, W_m), dtype=torch.int64, device=layer_vol_idx.device)
        mapped_img[layer_py, layer_px] = self._guidance_label[layer_vol_idx]
