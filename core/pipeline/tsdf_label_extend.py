...
def tsdf_label_extend(args: ProgramArgs):
    dataset = get_dataset(
    ...
    save_dir = os.path.join(args.save_dir, "tsdf_label_extend_" + args.extractor)
    ...
    print("Performing TSDF label extend")
    for idx in tqdm(range(len(dataset["scan_dataset"]))[args.start : args.end : args.stride]):
        if args.save_extraction:
            instance_pth = os.path.join(extraction_save_dir, "{:>06d}.pt".format(idx))
        ...
        tsdf_volume.fuse_label_into_voxel(
            masks=masks,
    ...
    tsdf_volume.save_instance(os.path.join(save_dir, "labels.pt"))
